function replaceText(node:Node){
  ...
  while(current){
    if(current.nodeValue?.includes('€'))current.nodeValue=current.nodeValue.replaceAll('€','$');
  ...
  for(const attribute of ['aria-label','title','placeholder']){
    ...
    node.querySelectorAll<HTMLElement>(`[${attribute}*="€"]`).forEach(element=>{
      const value=element.getAttribute(attribute);
      if(value)element.setAttribute(attribute,value.replaceAll('€','$'));
    });
  }
}
